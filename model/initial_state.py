...
class PackagesInHubsInitialStateSetter(InitialStateSetter):
    """An InitialStateSetter implementation that places agents randomly on the grid."""
    ...
    def set_initial_state(self, model: DroneModel) -> None:
        ...
        packages = []
        for i in range(model.num_packages):
            cell = model.random.choice(hubs).cell
            p = Package(model, cell, 0.5, 2, drop_zones[i])
        ...
        drones = []
        for _ in range(model.num_drones):
            hub = model.random.choice(hubs)
            cell = hub.cell
            assigned_packages = []
            for package in [agent for agent in cell.agents if isinstance(agent, Package)]:
                if not package.assigned:
                    package.assigned = True
                    assigned_packages.append(package)
                    break
            d = Drone(model, cell=cell, assigned_packages=assigned_packages)
            drones.append(d)
            hub.collect_drone(d)
...
def get_initial_state_setter_instance(name: str) -> InitialStateSetter:
    ...
    elif name == "packages_in_hubs":
        return PackagesInHubsInitialStateSetter()
    else:
        return None